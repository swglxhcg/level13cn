define(['ash', 'text/Text', 'game/constants/TextConstants', 'game/constants/ItemConstants'], function (Ash, Text, TextConstants, ItemConstants) {
	...
	var LogConstants = {
		...
		getLostItemMessage: function (resultVO) {
			...
			template.msg = "丢失 " + template.msg + ". ";

			var intros = [];
			switch (resultVO.action) {
				default:
					intros.push("差点掉到街上的裂缝里");
					intros.push("从腐烂的地板上掉下来");
					intros.push("爬栅栏时掉落物品");
					intros.push("偶然发现了一些破损的管道");
					intros.push("袋子口袋开着，一些东西掉了出来");
					intros.push("被阴影吓到了就跑了，留下了一些东西");
					break;
			}
		...
		getCraftItemMessage: function (itemVO) {
			...
			return "制造 " + Text.addArticle(itemVO.name).toLowerCase() + "." + itemDetails;
		},
