define([
	...
	var Level13 = Ash.Class.extend({
		...
		handleException: function (ex) {
			...
			// show popup
			let pos = GameGlobals.playerActionFunctions.playerPositionNodes.head ? GameGlobals.playerActionFunctions.playerPositionNodes.head.position : "(未知)";
			let bugTitle = StringUtils.encodeURI("[JS 错误] " + desc.title);
			let bugBody = StringUtils.encodeURI(
			   "详细信息:\n[在这里填写任何你认为有助于追踪漏洞的细节，比如在漏洞发生之前你在游戏中做了什么.]" +
			   "\n\n种子: " + GameGlobals.gameState.worldSeed + "\n位置: " + pos + "\n堆叠追踪:\n" + desc.stack);
			...
			GameGlobals.uiFunctions.showQuestionPopup(
				"错误",
				"你发现了一个bug!请重新加载页面以继续游玩. " +
				"如果重新加载不起作用，你可以清除你的数据并重新开始游戏，但你会失去所有的进度.<br\><br\>" +
				"您还可以通过以下方式帮助开发人员 <a href='" +
				url +
				"' target='_blank'>报告</a> 问题在GitHub.",
				"reload",
				"clear data",
				() => { location.reload(); },
