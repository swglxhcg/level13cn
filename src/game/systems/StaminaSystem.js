	...
	var StaminaSystem = Ash.System.extend({
		...
		updateStaminaValue: function (node, time) {
			...
			if (node.position.inCamp) {
				addAccumulation("基本", 2 * staminaComponent.health / 10 / 60);
			}
			if (isResting) {
				var timeLeft = busyComponent.getBusyTimeLeft();
				addAccumulation("休息中", Math.floor((maxVal - staminaComponent.stamina) / timeLeft * 4) / 4);
			}
			if (isHealing) {
				var timeLeft = busyComponent.getBusyTimeLeft();
				addAccumulation("治疗", Math.floor((maxVal - staminaComponent.stamina) / timeLeft * 4) / 4);
			}
		...
		updateStaminaWarning: function (node, time) {
			...
			if (isWarning && !this.isWarning) {
				...
				if (!node.position.inCamp) {
					if (hasCamp)
						logComponent.addMessage(LogConstants.MSG_ID_STAMINA_WARNING, "累了。应该马上回营地.");
					else
						logComponent.addMessage(LogConstants.MSG_ID_STAMINA_WARNING, "累了。应该尽快找个地方休息.");
				}
			}
