...
define([
	...
	var FaintingSystem = Ash.System.extend({
		...
		despair: function () {
			...
			var msgAdjective = hasWater ? (hasFood ? "筋疲力尽" : "饥饿") : "口渴";
			var msgMain = "";
			var msgLog = "";
			...
			if (hasCampOnLevel && this.lastVisitedCampNodes.head && this.lastVisitedCampNodes.head.camp.population >= 1 && Math.random() < 0.2) {
				msgMain = "虚弱并且 " + msgAdjective + ", 你坐下来休息。你的意识消失了。<br/>你在营地醒来。一些食腐动物发现了你，把你带回家.";
				msgLog = "世界在你眼中消失。你在营地醒来.";
			...
			if (hasFollowers && this.lastVisitedCampNodes.head && Math.random() < 0.1) {
				...
				msgMain = "虚弱并且 " + msgAdjective + ", 你坐下来休息。你的意识消失了。<br/>你在营地醒来。 <span class='hl-functionality'>" + follower.name + "</span> 带你回到了家.";
				msgLog = "世界在你眼中消失。你在营地醒来.";
				this.fadeOut(msgMain, msgLog, true, this.lastVisitedCampNodes.head.entity, 0, 0.5, 0, 0);
				return;
			}
		...
		fadeOutToOutside: function (msgAdjective) {
			...
			var msgMain = "虚弱并且 " + msgAdjective + ", 你坐下来休息。你的意识消失了。<br/>当你醒来时，你发现自己回到了一个熟悉的地方.";
			var msgLog = "世界在你眼前消失。你醒来后就不记得自己是怎么来的了.";
			if (nearestKnownSafeSector) {
				...
				if (GameGlobals.logWarnings) log.w("无处遁形.");
			}
		},
		
		fadeOutToLastVisitedCamp: function (handleResults, msgAdjective) {
			if (!this.lastVisitedCampNodes.head) return;
			var msgMain = "虚弱并且 " + msgAdjective + ", 你坐下来休息。你的意识消失了。<br/>当你醒来时，你发现自己回到了营地.";
			var msgLog = "世界在你眼前消失。你醒来后就不记得自己是怎么来的了.";
		...
		fadeOut: function (msg, msgLog, handleResults, sector, loseInventoryProbability, injuryProbability, loseAugmentationProbability, loseFollowerProbability) {
			...
			if (handleResults) {
				...
				GameGlobals.uiFunctions.showResultPopup("筋疲力尽", msg, resultVO, resultPopUpCallback);
			} else {
				finalStep();
			}
		},
		...
		teleport: function (sector) {
			...
			if (GameGlobals.logInfo) log.i("微弱的传送 " + sectorPosition);
			...
			setTimeout(function () {
				if (sector.has(CampComponent)) {
					...
					if (GameGlobals.logWarnings) log.w("昏厥目标扇区没有CampComponent");
				}
			}, 100);
		},
