define([
	...
	var WorkerSystem = Ash.System.extend({
		...
		updatePlayer: function (time) {
			...
			if (!isThirsty) {
				if (hasThirstPerk) {
					if (!inCamp) this.log("不再口渴.");
					perksComponent.removePerkById(PerkConstants.perkIds.thirst);
				}
			} else if (!hasThirstPerk) {
				if (!inCamp && (GameGlobals.gameState.unlockedFeatures.resources.water)) this.log("没有水!");
			...
			if (!isHungry) {
				if (hasHungerPerk) {
					if (!inCamp) this.log("不再饥饿.");
					perksComponent.removePerkById(PerkConstants.perkIds.hunger);
				}
			} else if (!hasHungerPerk) {
				if (!inCamp && (GameGlobals.gameState.unlockedFeatures.resources.food)) this.log("食物用完了!");
