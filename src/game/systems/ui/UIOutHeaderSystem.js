define([
	...
	var UIOutHeaderSystem = Ash.System.extend({
		...
		updatePlayerStats: function () {
			...
			this.updateStatsCallout("使探索更安全，清理更有效", "stats-vision", playerStatsNode.vision.accSources);
			...
			this.updateStatsCallout("决定最大耐力", "stats-health", playerStatsNode.stamina.healthAccSources, true);
			...
			this.updateStatsCallout("侦察所需", "stats-stamina", playerStatsNode.stamina.accSources);
			...
			if (campComponent && reputationComponent && maxPopulation > 0) {
				...
				var populationCalloutContent = "需要的声望等级:<br/>";
				populationCalloutContent += "当前: " + reqReputationCurrent + "<br/>";
				populationCalloutContent += "下一个工人到达需要: " + reqReputationNext;
		...
		updateScavengeAbility: function (showScavangeAbility, isInCamp, maxVision, shownVision, maxStamina) {
			...
			if (showScavangeAbility) {
				...
				if (scavengeEfficiency != this.scavangeAbilityLastUpdateValue) {
					...
					var scavengeEfficiencyExplanation = "<span class='info-callout-content-section-long'>在当前区域找到战利品的比率.</span>";
		...
		updateStatsCallout: function (description, indicatorID, changeSources, hideNumbers) {
			...
			if (!hideNumbers) {
				var totals = "总共: " + Math.round(total * 10000)/10000 + "/s";
		...
		updateGameMsg: function () {
			if (this.engine) {
				...
				if (this.autoPlayNodes.head) gameMsg += "自动游玩";
				if (GameGlobals.gameState.isPaused) gameMsg += "暂停";
		...
		updateLocation: function () {
			...
			$("#out-position-indicator").text("位置: " + positionText);
		},
		...
		updateHeaderTexts: function () {
			...
			var campComponent = this.currentLocationNodes.head.entity.get(CampComponent);
			var isInCamp = playerPosition.inCamp;
			var headerText = isInCamp && campComponent ? campComponent.getName() + " (等级 " + playerPosition.level + ")" : "level " + playerPosition.level;
