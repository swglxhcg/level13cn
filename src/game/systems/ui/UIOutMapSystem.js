	...
	var UIOutMapSystem = Ash.System.extend({
		...
		updateMapCompletionHint: function () {
			...
			if (level == surfaceLevel) {
				levelTypeText = "这一层是在城市的表面. "
			} else if (level == groundLevel) {
				levelTypeText = "这一层在地面上. ";
				...
				switch (levelComponent.notCampableReason) {
					...
					case LevelConstants.UNCAMPABLE_LEVEL_TYPE_POLLUTION:
						levelTypeText = "这一层被污染了. ";
						break;
					default:
						levelTypeText = "这一层无人居住. ";
						break;
				}
			} else {
			...
			let levelPronoun = levelTypeText.length > 0 ? "这个" : "这一层";
			let levelLocation = levelTypeText.length > 0 ? "" : " 在这一层";
			
			let mapStatus = GameGlobals.levelHelper.getLevelStats(level);
			var mapStatusText = levelLocation +  "仍有许多没有走过的街道" + ".";
			if (mapStatus.percentClearedSectors >= 1)
				mapStatusText = levelPronoun + " 已经被深入研究过了.";
			else if (mapStatus.percentScoutedSectors >= 1)
				mapStatusText =  levelPronoun + " 已经绘制了地图，但还有未探索的地方.";
			else if (mapStatus.percentRevealedSectors >= 1)
				mapStatusText = "仍有无人侦察的街道" + levelLocation + ".";
			else if (mapStatus.percentRevealedSectors >= 0.5)
				mapStatusText = "还有一些没有到过的街道" + levelLocation + ".";
		...
		getCollectorsText: function (sector, isScouted) {
			...
			var result2 = [];
			if (isScouted) {
				if (collectorFood.count === 1) result1.push("1个陷阱");
				if (collectorFood.count > 1) result1.push("陷阱");
				if (collectorFood.count == 0 && featuresComponent.resourcesCollectable.food > 0) result2.push ("食物")
				if (collectorWater.count === 1) result1.push("1个水桶");
				if (collectorWater.count > 1) result1.push("水桶");
				if (collectorWater.count == 0 && featuresComponent.resourcesCollectable.water > 0) result2.push ("水")
			}
			...
			let part2 = "";
			if (result2.length > 0) part2 = " (可以收集 " + result2.join(", ") + ")";
			return part1 + part2;
		},
		...
		getEnvironmentText: function (sector, isScouted) {
			...
			if (featuresComponent.sunlit) result.push("阳光照射");
			if (hazards.radiation > 0) result.push("放射性 (" + hazards.radiation + ")");
			if (hazards.poison > 0) result.push("污染 (" + hazards.poison + ")");
			if (hazards.cold > 0) result.push("冷 (" + hazards.cold + ")");
			if (hazards.debris > 0) result.push("碎片");
