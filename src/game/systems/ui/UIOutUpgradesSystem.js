define([
	...
	var UIOutUpgradesSystem = Ash.System.extend({
		...
		refresh: function () {
			...
			$("#world-upgrades-count").text("已经研究的升级: " + this.lastUpdateUpgradeCount);
		},
		...
		refreshTechDetails: function () {
			...
			if (hasSelection) {
				...
				var statusS = isUnlocked ? "已研究" : isAvailable ? "可用" : "未解锁";
		...
		getUpgradeTR: function (upgradeDefinition, status) {
			...
			switch (status) {
				...
				case UpgradeStatusEnum.UNLOCKABLE:
					...
					buttonTD = "<td class='minwidth'><button class='action' action='" + action + "' baseaction='" + baseActionID + "'>研究</button></td>";
					break;
				case UpgradeStatusEnum.BLUEPRINT_USABLE:
					 buttonTD = "<td class='minwidth'><button class='action' action='unlock_upgrade_" + upgradeDefinition.id + "'>解锁</button></td>";
					 break;
				case UpgradeStatusEnum.BLUEPRINT_IN_PROGRESS:
					...
					buttonTD = "<td class='list-action'><button class='action multiline' action='create_blueprint_" + upgradeDefinition.id + "'>组合</button></td>";
					break;
				default:
					buttonTD = "<td></td>";
					break;
			...
			if (status == UpgradeStatusEnum.BLUEPRINT_USABLE || status == UpgradeStatusEnum.BLUEPRINT_IN_PROGRESS) {
				...
				if (improvedBuildings.length > 0) {
					for (let i in improvedBuildings) {
						effects += "改良的 " + this.getImprovementDisplayName(improvedBuildings[i]).toLowerCase();
						effects += ", ";
					}
				}
				...
				if (improvedOccurrences.length > 0) {
					for (let i in improvedOccurrences) {
						effects += "改良的 " + improvedOccurrences[i];
					}
					effects += ", ";
				}
				...
				if (unlockedActions.length > 0) {
					for (let i in unlockedActions) {
						...
						effects += "启用 " + TextConstants.getActionName(baseActionID);
					}
					effects += ", ";
				}
