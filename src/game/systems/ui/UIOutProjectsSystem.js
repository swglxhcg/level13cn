define([
	...
	var UIOutProjectsSystem = Ash.System.extend({
		...
		update: function () {
			...
			this.elements.tabHeader.text("建筑项目");
		},
		...
		getProjectTR: function (project, isAvailable) {
			...
			if (isPassage) {
				...
				info = "在 "+location+"，连接 <span class='hl-functionality'>" + levels[0] + "</span> 层和 <span class='hl-functionality'>" + levels[1] + "</span> 层 " ;
			}
			if (project.action == "clear_debris_e" || project.action == "clear_debris_l" || project.action == "bridge_gap") {
				...
				info = "在 " + location + " 和 " + neighbourLocation + "中间， 在 " + project.level+" 层";
			}
			...
			if (isAvailable) {
				...
				if (project.action == "clear_debris_e" || project.action == "clear_debris_l" || project.action == "bridge_gap") {
					result += "<button class='btn-mini btn-meta hide-project' data-project='" + projectID + "'>隐藏</button>";
				}
				result += "</td>";
		...
		getMapLinkButton: function (sector) {
			return "<button class='btn-mini navigation' data-sector='" + sector + "'>地图</button>";
		},
