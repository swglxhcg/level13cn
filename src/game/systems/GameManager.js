	...
	var GameManager = Ash.Class.extend({
		...
		showSaveWarning: function (saveVersion) {
			...
			GameGlobals.uiFunctions.showQuestionPopup(
				"警告",
				"无法加载部分保存。很可能您的保存是旧的，并且与当前版本不兼容。重新开始游戏或继续你自己的冒险.<br><br/>保存版本: " + saveVersion + "<br/>当前版本: " + currentVersion,
				"重启",
				"继续",
				function () {
		...
		showVersionWarning: function (saveVersion, continueCallback) {
			...
			var message = "";
			message += "您的保存版本与当前版本不兼容。很有可能你上次玩游戏后游戏已经更新了. 查看 " + changelogLink + " 获取更多信息."
			message += "<br><br/>";
			message += "保存版本: " + saveVersion + "<br/>当前版本: " + currentVersion;
			message += "<br><br/>";
			message += " 重新开始游戏或继续你自己的冒险.";
			GameGlobals.uiFunctions.showQuestionPopup(
				"升级",
				message,
				"重启",
