	...
	function (Ash, ExceptionHandler, GameGlobals, GlobalSignals, GameConstants, CampConstants, UIConstants, ItemConstants, PlayerActionConstants, PositionConstants, UIPopupManager, ResourcesVO, MathUtils) {
		...
		var UIFunctions = Ash.Class.extend({
			...
			names: {
				resources: {
					stamina: "耐力所需",
					resource_metal: "金属metal",
					resource_fuel: "燃料fuel",
					resource_rubber: "橡胶rubber",
					resource_rope: "绳子rope",
					resource_food: "食物food",
					resource_water: "水water",
					resource_concrete: "混凝土concrete",
					resource_herbs: "药草herbs",
					resource_medicine: "药品medicine",
					resource_tools: "工具tools",
					item_exploration_1: "开锁器lock pick",
					rumours: "传闻rumours",
					evidence: "证据evidence",
				}
			},
			...
			registerActionButtonListeners: function (scope) {
				...
				$.each($(scope + " button.action"), function () {
					...
					if ($element.hasClass("click-bound")) {
						log.w("尝试绑定，点击两次! id: " + $element.attr("id"));
						return;
					}
			...
			generateCallouts: function (scope) {
				...
				$.each($(scope + " .info-callout-target"), function () {
					...
					if (generated) {
						log.w("信息标注已经生成! id: " + $target.attr("id") + ", 范围: " + scope);
						log.i($target);
						return;
					}
			...
			generateActionButtonCallout: function (action) {
				...
				*/
				...
				if (hasCosts) {
					...
					if (content.length > 0 || enabledContent.length) enabledContent += "<hr/>";
					enabledContent += "<span class='action-cost p-meta'>空闲</span><br />";
				}

				var duration = PlayerActionConstants.getDuration(baseActionId);
				if (duration > 0) {
					if (content.length > 0 || enabledContent.length) enabledContent += "<hr/>";
					enabledContent += "<span class='action-duration'>持续: " + Math.round(duration * 100) / 100 + "秒</span>";
				}
				...
				if (injuryRiskMax > 0 || inventoryRiskMax > 0 || fightRiskMax > 0) {
					if (content.length > 0 || enabledContent.length) enabledContent += "<hr/>";
					var inventoryRiskLabel = action === "绝望" ? "失去物品" : "项目丢失";
					if (injuryRiskMax > 0)
						enabledContent += "<span class='action-risk action-risk-injury warning'>伤害: <span class='action-risk-value'></span>%</span>";
					...
					if (fightRiskMax > 0)
						enabledContent += "<span class='action-risk action-risk-fight warning'>攻击: <span class='action-risk-value'></span>%</span>";
				}
				...
				if (content.length > 0) {
					...
					log.w("不能为带有动作的动作按钮创建标注 " + action + ". 没有标注内容.");
					return "";
				}
			},
						...
						switch (key) {
							case "improvementsOnLevel":
								...
								for (let improvementID in specialReqs[key]) {
									...
									s += rangeText + " " + displayName + " 在层 (" + count + ")";
								}
								break;
							default:
			...
			getGameInfoDiv: function () {
				var html = "";
				html += "<span id='changelog-version'>版本 " + GameGlobals.changeLogHelper.getCurrentVersionNumber() + "<br/>升级 " + GameGlobals.changeLogHelper.getCurrentVersionDate() + "</span>";
				html += "<p>请注意，这款游戏仍处于开发阶段，许多功能尚不完整且不平衡。更新可能会破坏保存。欢迎反馈和bug报告!</p>";
				html += "<p>Feedback:<br/>" + GameConstants.getFeedbackLinksHTML() + "</p>";
				html += "<p>更多信息:<br/><a href='faq.html' target='faq'>faq</a> | <a href='changelog.html' target='changelog'>changelog</a></p>";
				return html;
			},
			...
			showResultPopup: function (title, msg, resultVO, callback) {
				this.popupManager.showPopup(title, msg, "继续", false, resultVO, callback);
			},
			...
			showConfirmation: function (msg, callback, isMeta) {
				...
				this.popupManager.showPopup("确认？", msg, "确认", "取消", null, okCallback, cancelCallback, isMeta);
			},
			...
			showInput: function (title, msg, defaultValue, allowCancel, confirmCallback, inputCallback) {
				...
				let cancelButtonLabel = allowCancel ? "取消" : null;
				this.popupManager.showPopup(title, msg, "确认", cancelButtonLabel, null, okCallback);
