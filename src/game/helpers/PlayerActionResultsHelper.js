		...
		getRewardDiv: function (resultVO, isFight) {
			...
			if (resultVO.gainedFollowers && resultVO.gainedFollowers.length > 0) {
				for (let i = 0; i < resultVO.gainedFollowers.length; i++) {
					...
					div += "<br/>";
					div += "见到 <span class='hl-functionality'>" + Text.addArticle(followerTypeName) + "</span> 叫 " + follower.name + ". ";
					if (willJoin) {
						div += Text.capitalize(pronoun) + " 加入party.";
					} else if (this.nearestCampNodes.head) {
						div += Text.capitalize(pronoun) +" 将在 " + this.nearestCampNodes.head.camp.getName() + "见你， 在 " + this.nearestCampNodes.head.position.level + "层.";
					}
					div += "</div>";
				}
			}
			...
			if (resultVO.gainedEvidence) {
				gainedhtml += "<li>" + resultVO.gainedEvidence + " 见识</li>";
			}
			if (resultVO.gainedRumours) {
				gainedhtml += "<li>" + resultVO.gainedRumours + " 传闻</li>";
			}
			if (resultVO.gainedFavour) {
				gainedhtml += "<li>" + resultVO.gainedFavour + " 支持</li>";
			}
			if (resultVO.gainedPopulation) {
				gainedhtml += "<li>" + resultVO.gainedPopulation + " 人口</li>";
			}
			...
			if (resultVO.gainedCurrency) {
				gainedhtml += "<li>" + resultVO.gainedCurrency + " 银币</li>";
			}
			...
			if (resultVO.lostResources.getTotal() > 0 || resultVO.lostItems.length > 0 || resultVO.lostCurrency > 0) {
				var lostMsg = resultVO.lostItems.length > 1 ? "失去一些东西." : resultVO.lostItems.length > 0 ? "失去一个东西." : ""
			...
			if (resultVO.gainedResources.getTotal() > 0 || resultVO.gainedItems.length > 0 || !isInitialSelectionValid) {
				...
				baghtml += "<ul></ul>";
				baghtml += "<p class='msg-empty p-meta'>" + (isFight ? "对手一无所有." : "这儿已没有留下什么.") + "</p>";
				baghtml += "</div>"

				baghtml += "<div id='resultlist-inventorymanagement-kept' class='infobox inventorybox'>";
				baghtml += "<ul></ul>";
				baghtml += "<p class='msg-empty p-meta'>你的 " + (hasBag ? "包" : "口袋") + " 是空的.</p>";
				baghtml += "</div>"
			...
			if (!hasGainedStuff && !hasLostStuff) {
				if (isFight) div += "<p class='p-meta'>什么也没遗落.</p>"
				else if (resultVO.action === "despair") div += "";
				else if (resultVO.action === "clear_workshop") div += "";
				else if (resultVO.hasCustomReward) div += "";
				else div += "<p class='p-meta'>没找到有用的东西.</p>";
			}
			
			if (resultVO.lostFollowers && resultVO.lostFollowers.length > 0) {
				for (let i = 0; i < resultVO.lostFollowers.length; i++) {
					div += "<p class='warning'><span class='hl-functionality'>" + resultVO.lostFollowers[i].name + "</span> 留下.</p>";
				}
			}

			if (resultVO.gainedInjuries.length > 0) {
				div += "<p class='warning'>你受伤了.</p>";
			}

			if (resultVO.lostPerks.length > 0) {
				div += "<p class='warning'>你失去 " + TextConstants.getListText(resultVO.lostPerks.map(perkVO => perkVO.name)) + ".</p>";
			}

			if (resultVO.lostCurrency > 0) {
				div += "<p class='warning'>你失去 " + resultVO.lostCurrency + " 银币.</p>";
			}
		...
		logResults: function (rewards) {
			...
			if (rewards) {
				if (rewards.gainedBlueprintPiece) {
					if (!this.tribeUpgradesNodes.head.upgrades.hasUpgrade(rewards.gainedBlueprintPiece)) {
						...
						if (blueprintVO.currentPieces === 1) {
							logComponent.addMessage(LogConstants.MSG_ID_FOUND_BLUEPRINT_FIRST, "发现了一件被遗忘的技术.");
						}
					}
				}
				...
				if (rewards.selectedItems) {
					for (let i = 0; i < rewards.selectedItems.length; i++) {
						...
						if (itemsComponent.getCountById(item.id, true) === 1) {
							...
							logComponent.addMessage(LogConstants.MSG_ID_FOUND_ITEM_FIRST, "找到一个 " + item.name + ".");
						}
					}
				}
					
				if (rewards.gainedFollowers && rewards.gainedFollowers.length > 0) {
					logComponent.addMessage(LogConstants.getUniqueID(), "见到一个新的追随者.");
				}
				...
				if (rewards.lostFollowers && rewards.lostFollowers.length > 0) {
					logComponent.addMessage(LogConstants.MSG_ID_LOST_FOLLOWER, "失去 " + rewards.lostFollowers.length + " 追随者.");
				}
