			...
			var checkRequirementsInternal = function (action, sector) {
				...
				if (requirements) {
					if (requirements.vision) {
						let result = this.checkRequirementsRange(requirements.vision, playerVision, "{min} 视野需求", "{max} 最大视野");
						if (result) {
							return result;
						}
					}
					...
					if (requirements.stamina) {
						let result = this.checkRequirementsRange(requirements.stamina, playerStamina, "{min} 耐力需求", "{max} 最大耐力");
						if (result) {
							return result;
						}
					}
					...
					if (typeof requirements.maxStamina !== "undefined") {
						...
						if (currentValue !== requiredValue) {
							if (currentValue) reason = "已经完全休息了.";
							else reason = "必须充分休息.";
							return {value: 0, reason: reason};
						}
					}
					...
					if (requirements.health) {
						...
						if (playerHealth < requirements.health) {
							reason = requirements.health + " 健康度需要.";
					...
					if (typeof requirements.sunlit !== "undefined") {
						...
						if (currentValue !== requiredValue) {
							if (currentValue) reason = "阳光光照条件不允许.";
							else reason = "阳光需求.";
							return { value: 0, reason: reason };
						}
					}
					...
					if (requirements.population && !shouldSkipCheck(PlayerActionConstants.UNAVAILABLE_REASON_POPULATION)) {
						...
						let result = this.checkRequirementsRange(requirements.population, currentPopulation, "{min} 人口需求", "最大值 {max} 人口", "工人需求", "没有可以的工人");
						if (result) {
							return result;
						}
					}
					...
					if (typeof requirements.rumourpoolchecked != "undefined") {
						if (campComponent) {
							...
							if (requirements.rumourpoolchecked != campValue) {
								if (!requirements.rumourpoolchecked) reason = "目前没有新的传闻.";
								if (requirements.rumourpoolchecked) reason = "有新的传闻.";
								return { value: 0, reason: reason };
							}
						}
					}
					...
					if (requirements.numCamps) {
						...
						if (requirements.numCamps > currentCamps) {
							reason = requirements.numCamps + " 需要营地.";
					...
					if (typeof requirements.inCamp !== "undefined") {
						...
						if (required !== current) {
							if (required) {
								...
								return { value: 0, reason: "必须在外面做这个." };
							}
						}
					}
					...
					if (requirements.improvementsOnLevel) {
						...
						for (var improvementID in improvementRequirements) {
							...
							var range = improvementRequirements[improvementID];
							let result = this.checkRequirementsRange(range, amount,
								"{min}x " + displayName + " 需要在该层",
								"最大 {max} " + displayName + " 在此层",
								displayName + " 需要在这层中",
								displayName + " 已在这层建造"
							);
							if (result) {
					...
					if (requirements.workers) {
						...
						for (let workerType in workerRequirements) {
							...
							let result = this.checkRequirementsRange(range, amount, workerType + " 需求", "没有 " + workerType + " 需求");
							if (result) {
								return result;
							}
						}
					...
					if (requirements.perkEffects) {
						...
						for(let perkType in perkRequirements) {
							...
							if ((!isOneValue && (min > totalEffect || max <= totalEffect)) || (isOneValue && validPerk == null)) {
								if (min > totalEffect) reason = "当发生此事时不能做: " + perkType;
								if (max <= totalEffect) reason = "地位的需要: " + perkType;
								return { value: 0, reason: reason };
							}
						}
					}
					...
					if (requirements.perks) {
						...
						for (let perkID in perkRequirements) {
							...
							if (requiredValue != actualValue) {
								...
								if (requiredValue) {
									return { value: 0, reason: "地位的需要: " + perk.name };
								} else {
									return { value: 0, reason: "利用地位阻塞: " + perk.name };
								}
							}
						}
					}
					...
					if (requirements.upgrades) {
						...
						for (var upgradeID in upgradeRequirements) {
							...
							if (requirementBoolean != hasBoolean) {
								...
								if (requirementBoolean) reason = "升级所需: " + name;
								else reason = "已经研究了升级 (" + name + ")";
								return { value: 0, reason: reason };
							}
						}
					}
					...
					if (requirements.unlockedWorkers) {
						for (let workerID in requirements.unlockedWorkers) {
							...
							if (requiredValue != actualValue) {
								if (requiredValue) {
									return { value: 0, reason: "工人需求: " + workerID };
								} else {
									return { value: 0, reason: "工人已解锁: " + workerID };
								}
							}
						}
					}
					...
					if (typeof requirements.path_to_camp !== "undefined") {
						...
						if (currentValue !== requiredValue) {
							if (currentValue) reason = "有通往营地的路";
							else reason = "没有通往营地的路.";
							return { value: 0, reason: reason };
						}
					}
					...
					if (requirements.followers) {
						if (typeof requirements.followers.maxRecruited !== "undefined") {
							...
							let result = this.checkRequirementsBoolean(requiredValue, currentValue, "招募的最大追随者数", "未招募的最大追随者数");
					...
					if (typeof requirements.bag !== "undefined") {
						if (requirements.bag.validSelection) {
							...
							if (validStart && !validNow) {
								return { value: 0, reason: "搬不动那么多东西." };
							}
						}
						...
						if (typeof requirements.bag.full !== "undefined") {
							...
							if (requiredValue !== currentValue) {
								if (requiredValue)
									return {value: 0, reason: "包必须装满."};
								else
									return {value: 0, reason: "包满了."};
							}
						}
					}
					...
					if (requirements.outgoingcaravan) {
						if (typeof requirements.outgoingcaravan.available !== "undefined") {
							...
							if (requiredValue > currentValue) {
								return {value: 0, reason: "没有可用的商队大篷车."};
							}
						}
						if (typeof requirements.outgoingcaravan.validSelection !== "undefined") {
							...
							if (requiredValue != currentValue) {
								if (requiredValue)
									return {value: 0, reason: "无效的选择."};
								else
									return {value: 0, reason: "有效的选择."};
							}
						}
					}
					...
					if (requirements.camp) {
						...
						if (typeof requirements.camp.raid !== "undefined") {
							...
							if (requiredValue != currentValue) {
								return { value: 0, reason: (requiredValue ? "目前没有袭击" : "有人突袭" ) };
							}
						}
					}
					...
					if (requirements.sector) {
						...
						if (typeof requirements.sector.hasCamp !== "undefined") {
							...
							if (value !== requiredValue) {
								var reason = requiredValue ? "这里没有营地." : "这里有营地";
								return { value: 0, reason: reason };
							}
						}
						...
						if (requirements.sector.canHaveCamp) {
							if (!featuresComponent.canHaveCamp()) {
								return { value: 0, reason: "地点不适合建造营地" };
							}
						}
						if (typeof requirements.sector.enemies != "undefined") {
							...
							if (enemiesComponent.hasEnemies != requirements.sector.enemies) {
								if (requirements.sector.enemies)
									return { value: 0, reason: "通过区域需要对付的敌人" };
								else
									return { value: 0, reason: "这里太危险了" };
							}
						}
						if (typeof requirements.sector.scouted != "undefined") {
							if (statusComponent.scouted != requirements.sector.scouted) {
								if (statusComponent.scouted)    reason = "已勘察过的地区.";
								else                            reason = "这片区域还没有勘察过.";
								return { value: 0, reason: reason };
							}
						}
						if (typeof requirements.sector.scavengedPercent != "undefined") {
							...
							let result = this.checkRequirementsRange(range, currentVal, "", "这个地区已被清理干净.");
							if (result) {
								return result;
							}
						}
						if (typeof requirements.sector.spring != "undefined") {
							if (featuresComponent.hasSpring != requirements.sector.spring) {
								if (featuresComponent.hasSpring)    reason = "有一个源泉.";
								else                                reason = "这里没有源泉.";
								return { value: 0, reason: reason };
							}
						}
						if (typeof requirements.sector.scoutedLocales !== "undefined") {
							for(var localei in requirements.sector.scoutedLocales) {
								...
								if (requiredStatus !== currentStatus) {
									if (requiredStatus) reason = "必须对区域进行侦察.";
									if (!requiredStatus) reason = "区域已经勘测过了.";
									return { value: 0, reason: reason };
								}
							}
						}
						for (let i in PositionConstants.getLevelDirections()) {
							...
							if (typeof requirements.sector[blockerKey] !== 'undefined') {
								...
								if (requiredValue !== currentValue) {
									if (currentValue) {
										return { value: 0, reason: "此路不通. " + movementOptionsComponent.cantMoveToReason[direction] };
									} else {
										return { value: 0, reason: "没有任何东西阻碍移动到 " + directionName + "." };
									}
								}
							}
							...
							if (typeof requirements.sector[clearedKey] !== 'undefined') {
								...
								if (requiredValue !== currentValue) {
									if (currentValue) {
										return { value: 0, reason: "浪费了. " };
									} else {
										return { value: 0, reason: "废物未被清除： " + directionName + "." };
									}
								}
							}
						}
						...
						if (typeof requirements.sector.passageUp != 'undefined') {
							if (!passagesComponent.passageUp) {
								reason = "没有通道.";
								return { value: 0, reason: "此路不通. " + reason };
							} else {
						...
						if (typeof requirements.sector.acessible_to_workers != "undefined") {
							...
							if (currentValue != requiredValue) {
								var reason = requiredValue ? "工人无法使用、接近" : "工人可以使用、接近";
								return { value: 0, reason: reason };
							}
						}
						...
						if (requirements.sector.hazards) {
							for (var hazard in requirements.sector.hazards) {
								...
								let result = this.checkRequirementsRange(range, currentVal, "最小值 {min} " + hazard, "最大值 {max} " + hazard);
								if (result) {
									return result;
								}
							}
						...
						if (requirements.sector.buildingDensity) {
							let result = this.checkRequirementsRange(requirements.sector.buildingDensity, featuresComponent.buildingDensity, "区域建设过于稀少", "建筑过于密集的区域");
							if (result) {
								return result;
							}
						}
					...
					if (requirements.uses_in_fight) {
						...
						for (var itemID in usesReqs) {
							...
							if (min > current) {
								return { value: 0, reason: "必须先使用 " + itemName + " 。" };
							} else if (max <= current) {
								if (itemID == actionItemID && max == 1) {
									return { value: 0, reason: "已经使用" };
								} else {
									return { value: 0, reason: "已经使用 " + itemName };
								}
							}
						}
					}
		...
		getImproveActionDescription: function (action) {
			...
			switch (action) {
				...
				case "improve_in_market":
					return isNextLevelMajor ? "增加传闻的产生和每次访问的传闻" : "增加传闻的产生";
				case "improve_in_shrine":
					return isNextLevelMajor ? "增加声望奖励和冥想成功率" : "增加声望奖励";
				case "improve_in_inn":
					return isNextLevelMajor ? "增加传闻的产生和最大的追随者" : "增加传闻的产生";
			}
			
			return "增加 " + improvementName;
		},
		...
		getMinimumCampAndStep: function (action) {
			...
			if (costs) {
				if (costs && costs.resource_fuel && costs.resource_fuel > 0) {
					addRequirement(WorldConstants.CAMP_ORDINAL_FUEL, WorldConstants.CAMP_STEP_POI_2, "燃料");
				}
				if (costs && costs.resource_rubber && costs.resource_rubber > 0) {
					addRequirement(WorldConstants.CAMP_ORDINAL_GROUND, WorldConstants.CAMP_STEP_POI_2, "橡胶");
				}
				if (costs && costs.resource_herbs && costs.resource_herbs > 0) {
					addRequirement(WorldConstants.CAMP_ORDINAL_GROUND, WorldConstants.CAMP_STEP_POI_2, "药草");
				}
			}
			...
			if (reqs && reqs.improvements) {
				for (var improvementID in reqs.improvements) {
					...
					addRequirement(buildCampStep.campOrdinal, buildCampStep.step, "需要建造");
				}
			}
			
			// deity
			if (reqs && reqs.deity) {
				addRequirement(WorldConstants.CAMP_ORDINAL_GROUND, WorldConstants.CAMP_STEP_POI_2, "神");
			}
